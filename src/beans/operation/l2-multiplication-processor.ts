...
import { L2Multiplication } from './l2-multiplication';

export class L2MultiplicationProcessor extends Bean implements L2OperationProcessor {
  pass = 2;
  ...
  process(c: L2ParseContext, t1: ProcessToken, t2?: ProcessToken, t3?: ProcessToken): ProcessResult {
    if (this.expressionReader.isOperand(t1) && L1Operator.matches(t2, '*') && this.expressionReader.isOperand(t3)) {
      ...
      return { step: new L2Multiplication(operand, combinePos(t1.pos, t3.pos)), skip: 2 };
    }
  }
}
