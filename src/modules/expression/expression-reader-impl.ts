...
import { ExpressionReader, ExpressionReaderContext, ReadExpressionOpts } from '@/modules/expression/expression-reader';
import { IdentifierResolver } from '@/modules/expression/identifier-resolver';
import { NumberLiteral } from '@/modules/expression/number-literal';
...
import { Number } from '@/modules/tokenizer/number';
...
export class ExpressionReaderImpl extends Bean implements ExpressionReader {
  ...
  private internalResolveOperand(c: ExpressionReaderContext, obj: Token | Expression) {
    ...
    if (obj instanceof Bracket && obj.start === '(') {
      const r = this.readList(
        { ...c, tokenReader: new TokenReader(obj.tokenList) },
        {
