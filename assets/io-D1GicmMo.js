...
const sourceCode = "export native function print(s: string);\nexport native function println(s: string);\nexport native function readln(): string;";
class LibraryIO extends Bean {
  constructor() {
    super(...arguments);
    this.moduleName = "lib:io";
  ...
  nativeMethods(c) {
    ...
    const getTerm = () => {
      if (!data.term) {
        ...
        Object.assign(data.div.style, {
          width: "100%",
          height: "100%"
        });
        data.term = new xtermExports.Terminal({
          cursorBlink: true,
    ...
    return {
      ...
      readln() {
        return new Promise((resolve) => {
          let input = "";
          const term = getTerm();
          const handler = term.onData((data2) => {
            switch (data2) {
              case "":
                if (input.length > 0) {
